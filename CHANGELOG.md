...
### Добавлено
- **Обёртки `run.cmd` и `run_report.cmd`** для Windows Task Scheduler. Перед запуском Python-скрипта делают `git pull --ff-only` и подтягивают актуальный код из GitHub. Если pull не удался — запускается локальная версия (отзывы важнее свежего кода), причина пишется в `git_pull.log`.
- README: инструкция по перенацеливанию существующих заданий Task Scheduler на новые `.cmd` через `schtasks /change`.

### Контекст
В период 28–30 апреля выяснилось, что прод запускал Python-скрипт напрямую и не подтягивал коммиты с GitHub. Изменения от 22.04 (vision always-on) фактически заработали только 27.04, когда репо обновили вручную. Обёртки решают это автоматически.
